...
<table class="tableE table-hover table-sm mx-auto mb-5" id="ba_{{ $business_area_id }}">
    <thead>
    <tr>
...
{{--            <small>({{ $presentation_data['titles']['last week'] }})--}}
            </small></th>
        <th>% Variance</th>
    </tr>
    </thead>
...
$items = [
    ...
    @foreach($items as $item)
        ...
        $variance = isset($current_week) && $prev_week != 0 && isset($prev_week) ? round((($current_week - $prev_week) / $prev_week) * 100, 2).'%' : '-';
...
<div id="escalation_tb"></div>
@include('frontend.business_goals.partials.table2xlsx_button', ['table_id' => 'ba_2_'. $business_area_id] )
<table class="tableE table-hover table-sm table-bordered mx-auto" id="ba_2_{{ $business_area_id }}">
    ...
    <tr>
        <th>Wk {{ $week_in_focus->id }}
{{--            <small>{{ $week_in_focus->from_day }} - {{ $week_in_focus->to_day }}</small>--}}
        </th>
        <th>Wk {{ $week_in_focus->id - 1 }}
...
$data = commercialCustRelationsArr();
    ...
    @foreach($data as $item)
        @php
            $field = $form_fields_collection->where('label', $item['Escalation type'].' (pending)')->first();
            $field_closed = $form_fields_collection->where('label', $item['Escalation type'].' (closed)')->first();
...
<div class="row mt-3">
    <div class="col-12">
        @include('frontend.business_goals.dailies._7')
    </div>
</div>
@endif

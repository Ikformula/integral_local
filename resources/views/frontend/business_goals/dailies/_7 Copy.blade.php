...
$items = [
    ...
    @foreach($items as $item)
        ...
        $variance = isset($current_week) && $current_week != 0 && isset($prev_week) ? round((($current_week - $prev_week) / $current_week) * 100, 2).'%' : '-';
...
<table class="table table-hover table-sm table-bordered" id="ba_2_{{ $business_area_id }}">
    ...
    <tr>
        <th>Wk {{ $week_in_focus->id }} <small>{{ $week_in_focus->from_day }} - {{ $week_in_focus->to_day }}</small></th>
        <th>Wk {{ $week_in_focus->id - 1 }} <small>{{ $presentation_data['titles']['last week'] }}</small></th>
        <th>Closed</th>
        <th>Pending</th>
    </tr>
    </thead>

                                ...
                                <div class="row mb-2">
                                    <div class="col">
                                        <div class="btn-group float-right" role="group">
                                            ...
                                            <button class="search-btn btn btn-outline-dark" data-search-text="absent">
                                                Absentees
                                            </button>
                                            <button class="search-btn btn btn-outline-dark"
                                                    data-search-text="closed early">
                                                Closed Early
                                            </button>
                                        </div>
                                    ...
                                    <tr>
                                        ...
                                        @if(isset($attendances[$staff_ara_id]))
                                            ...
                                            @foreach($dates as $date)
                                                    ...
                                                    <td style="background-color: @if(isset($attendances[$staff_ara_id][$date['str']]['day_attendance']['manager_auth'])) #a770d1 @php($manager_auth[$staff_ara_id][$date['str']] = $attendances[$staff_ara_id][$date['str']]['day_attendance']['manager_auth']) @else {{$attendances[$staff_ara_id][$date['str']]['day_attendance']['day_schedule']['colour']}} @php($manager_auth[$staff_ara_id][$date['str']] = false) @endif;

                                                @if($attendances[$staff_ara_id][$date['str']]['day_attendance']['closing_status'] == "closed early") border-bottom: 3px solid red; @endif"
                                                        @if($manager_auth[$staff_ara_id][$date['str']] != false)
                                                    ...
                                                    >
                                                        @if(!in_array($date['week_day'], ['Saturday', 'Sunday']) )
                                                            <small>@if($attendances[$staff_ara_id][$date['str']]['day_attendance']['day_schedule']['colour'] == '#F5F5F5')
                                                                    Working remotely
                                                                @elseif($manager_auth[$staff_ara_id][$date['str']] != false)
                                                                    @php($attendances[$staff_ara_id][$date['str']]['day_attendance']['status'] = 'authorized')

                                                                    @php($attendances[$staff_ara_id][$date['str']]['day_attendance']['closing_status'] = '')

                                                                    {{$attendances[$staff_ara_id][$date['str']]['day_attendance']['status']}}
                                                                @endif</small>
                                                            <br>
                                                        @endif

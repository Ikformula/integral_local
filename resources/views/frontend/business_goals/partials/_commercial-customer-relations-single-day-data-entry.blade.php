...
<table class="table table-bordered table-hover table-striped">
    ...
    <tbody>
        @foreach($comm_fields as $comm_field)
            ...
            @isset($data_points)
                @php
                    $field_value = $data_points->where('business_area_id', $business_area->id)->where('score_card_form_field_id', $field->id)->first();
                    // $closed_field_value = $data_points->where('business_area_id', $business_area->id)->where('score_card_form_field_id', $closed_field->id)->where('for_date', $for_date)->first();
                    $closed_field_value = $data_points->where('business_area_id', $business_area->id)->where('score_card_form_field_id', $closed_field->id)->first();
                @endphp
            @endisset
            <tr>
